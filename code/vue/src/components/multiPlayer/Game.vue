...
const currentUserTurn = computed(() => {
    ...
    return (
        ...
        storeGames.playerNumberOfCurrentUser(props.game)
    );
});
...
const cardBgColor = computed(() => {
    switch (props.game.gameStatus) {
        ...
        case 1:
        case 2:
            return storeGames.playerNumberOfCurrentUser(props.game) ==
...
const statusGameMessage = computed(() => {
    switch (props.game.gameStatus) {
        ...
        case 0:
            const figure =
                props.game.currentPlayer === 1 ? "(cross)" : "(circle)";
            return currentUserTurn.value
                ? "Your turn " + figure
                : "Opponent turn " + figure;
        case 1:
        case 2:
            return storeGames.playerNumberOfCurrentUser(props.game) ==
                props.game.gameStatus
...
<template>
    ...
    <Card class="relative grow mx-4 mt-8 pt-2 pb-4 px-1" :class="cardBgColor">
        ...
        <CardContent class="py-4 px-8">
            <h3
                class="pt-0 text-2xl font-bold py-2"
                :class="statusMessageColor"
            >
                {{ statusGameMessage }}
            </h3>
            <div>
