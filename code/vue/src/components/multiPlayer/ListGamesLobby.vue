        ...
        <div v-for="game in storeLobby.games" :key="game.id" class="flex ps-2 pe-1">
            ...
            <div class="flex flex-col grow">
                ...
                <span class="text-xs ps-1 pb-2 -mt-1 text-gray-500">
                    {{ new Date(game.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }) }}
                </span>
                <!-- Board Type -->
                <span class="text-xs ps-1 text-gray-500">
            ...
            <div class="py-1 flex items-center min-w-[1.9rem]">
                <button
                    v-show="storeLobby.canRemoveGame(game)"
                    type="button"
                ...
                <button
                    v-show="storeLobby.canJoinGame(game)"
                    type="button"
                    class="rounded bg-cyan-500 p-2 m-0.5 text-white"
                    @click="storeLobby.joinGame(game.id, game.board)"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="size-4">
                        <path
                            stroke-linecap="round"
