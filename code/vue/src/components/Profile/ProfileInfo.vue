...
<template>
    <div class="p-6 bg-white rounded-lg shadow-lg max-w-4xl mx-auto">
        <div class="flex items-center justify-between space-x-6">
            ...
            <div class="flex items-center space-x-6 flex-grow">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">Type: {{ props.user.type === 'P' ? 'Player' : 'Admin' }}</p>
                    <p v-if="props.user.type === 'P'" class="text-sm text-gray-600">Brain Coins: {{
                        props.user.brain_coins_balance }}</p>
                </div>
            </div>
            ...
            <div v-if="props.user.nickname !== authStore.user.nickname" class="flex flex-col space-y-2">
                ...
                <div>
                    <button v-if="!props.user.deleted_at" @click="showDeleteForm"
                ...
                <button v-if="!props.user.blocked && !props.user.deleted_at" @click="blockAccount"
                    class="px-6 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600">
                    Block Account
                </button>

                <button v-else-if="props.user.blocked && !props.user.deleted_at" @click="blockAccount"
                    class="px-6 py-2 bg-green-500 text-white rounded-md hover:bg-green-600">
                    Unblock Account
                </button>

                <!-- Restore Account Button -->
                <button v-if="props.user.deleted_at" @click="restoreAccount"
        ...
        <div v-if="formDelete" class="mt-6">
            <DeleteUserAsAdmin :user="props.user" @hidedeleteForm="hideDeleteForm"
                @accountDeleted="handleAccountDeleted" @userDeleted="handleUserDeleted" />
        </div>
    </div>
</template>
