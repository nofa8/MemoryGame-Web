...
const submitTransaction = async () => {
  ...
  try {
    ...
    if (error.response && error.response.status === 422) {
      errorMessage.value = error.response.data.message || 'An error occurred while making the purchase';
    } else {
...
<template>
  <div class="p-4">
    ...
    <div class="flex items-center mb-4">
      ...
      <input type="number" v-model="brainCoins" @input="updateEuros" class="mx-4 p-2 border rounded-md w-20 text-center"
        min="10" step="10" />
      <button @click="incrementCoins" class="px-4 py-2 bg-green-500 text-white rounded-md">+</button>
    </div>
    ...
    <div class="mb-4">
      ...
      <input type="text" id="paymentReference" v-model="paymentReference" class="p-2 border rounded-md w-full"
        placeholder="Enter payment reference" />
    </div>

    <!-- Botão para enviar -->
    <button @click="submitTransaction" class="px-4 py-2 bg-blue-600 text-white rounded-md">Confirm Purchase</button>
